...
starts and no more than three attempts for a transport-transient aggregate. An `allowFailure` response is promoted to
that aggregate path only when every ordered subcall failed with a transient RPC class; mixed or invariant results are
not retried. Other public discovery can retry
...
The hot poll reuses a validated source-target index and skips empty or unrelated Initialize maintenance. Protected
periodic cycles write the large source catalog once after all adapters advance, while retaining the last projected
cursors until the projection and runtime checkpoint commit in order. A crash may safely repeat a range but cannot
...
`/api/v1/agent/daily-profit` is a static read model generated by the five-minute business-report job, not a second
daemon. It is available when the operator laptop is off and exposes no password or write method. Native USDG/ETH
evidence remains primary; USD/CNY values are market-reference estimates and turn partial when quotes are missing or
disagree. The endpoint never includes `projectResultByAsset`, signing material, wallet identifiers or offchain costs.
