...
const rpcPort = Number(process.env.MANGA_WETH_TEST_RPC_PORT || 18_549)
const rpcUrl = `http://127.0.0.1:${rpcPort}`
const chain = defineChain({
  id: 31_337,
  name: 'PAIR WETH atomic-arbitrage deterministic test',
...
function compile() {
  const sources = {
    ...
    'WethAtomicArb.sol': {
      content: fs.readFileSync(path.join(root, 'contracts', 'WethAtomicArb.sol'), 'utf8'),
    },
...
async function main() {
  ...
  try {
    ...
    const executorArtifact = contracts['WethAtomicArb.sol'].WethAtomicArb
    ...
    const maximumAmountIn = parseEther('0.05')
    const minimumGrossProfit = parseEther('0.00001')
    const seed = parseEther('0.02')
    const deployHash = await walletClient.deployContract({
      abi: executorArtifact.abi,
      bytecode: `0x${executorArtifact.evm.bytecode.object}`,
      args: [operator, maximumAmountIn, minimumGrossProfit],
      value: seed,
    })
    const deployReceipt = await publicClient.waitForTransactionReceipt({ hash: deployHash })
    if (deployReceipt.status !== 'success' || !deployReceipt.contractAddress) {
      throw new Error('WETH deterministic deployment failed')
    }
    ...
    const seededWeth = await publicClient.readContract({
      address: WETH,
      abi: tokenAbi,
      functionName: 'balanceOf',
      args: [executor],
    })
    const strandedNative = await publicClient.getBalance({ address: executor })
    if (seededWeth !== seed || strandedNative !== 0n) {
      throw new Error('deployment seed was not wrapped into exact WETH principal')
    }

    await write(POOL_MANAGER, managerAbi, 'configureTakeMint', [true])
    const expectedProfit = parseEther('0.0001')
    await write(V3_ROUTER, routerAbi, 'configure', [expectedProfit])
    await write(V3_ROUTER, routerAbi, 'configureProfitToken', [WETH])
    const mockPools = [
      [WETH, USDG, 100, getAddress('0x1000000000000000000000000000000000000001')],
      [USDG, ENTRY_TOKEN, 500, getAddress('0x1000000000000000000000000000000000000002')],
      [EXIT_TOKEN, USDG, 500, getAddress('0x1000000000000000000000000000000000000003')],
      [WETH, ENTRY_TOKEN, 500, getAddress('0x1000000000000000000000000000000000000004')],
      [EXIT_TOKEN, WETH, 500, getAddress('0x1000000000000000000000000000000000000005')],
    ]
    for (const args of mockPools) await write(V3_FACTORY, factoryAbi, 'setPool', args)
    for (const [pool, mode, profit] of [
      [mockPools[3][3], 1, 0n],
      [mockPools[4][3], 2, expectedProfit],
    ]) {
      await publicClient.request({
        method: 'hardhat_setCode',
    ...
    await write(mockPools[4][3], poolArtifact.abi, 'configureOutputToken', [WETH])

    const route = {
      targetToken: TARGET_TOKEN,
      entryToken: ENTRY_TOKEN,
      exitToken: EXIT_TOKEN,
      entryV3Path: v3Path([WETH, USDG, ENTRY_TOKEN], [100, 500]),
      exitV3Path: v3Path([EXIT_TOKEN, USDG, WETH], [500, 100]),
      entryV4Pool: {
    ...
    const protectedProfit = parseEther('0.00005')
    const positiveArgs = [route, amountIn, protectedProfit, block.timestamp + 300n]
    ...
    if (simulation.result[0] !== amountIn + expectedProfit || simulation.result[1] !== expectedProfit) {
      throw new Error('WETH positive simulation did not produce the exact expected business result')
    }

    const directRoute = {
      ...route,
      entryV3Path: v3Path([WETH, ENTRY_TOKEN], [500]),
      exitV3Path: v3Path([EXIT_TOKEN, WETH], [500]),
    }
    const directArgs = [directRoute, amountIn, protectedProfit, block.timestamp + 300n]
    ...
    if (directSimulation.result[0] !== amountIn + expectedProfit || directSimulation.result[1] !== expectedProfit) {
      throw new Error('WETH direct-pool simulation did not produce the exact expected business result')
    }
    ...
    negativeChecks.push(
      await mustRevert(
        'amount_over_deployment_cap',
        'InvalidAmount',
        'InvalidAmount()',
        executeRequest([route, maximumAmountIn + 1n, protectedProfit, block.timestamp + 300n]),
      ),
    )
    negativeChecks.push(
      await mustRevert(
        ...
        'ProfitFloorTooLow',
        'ProfitFloorTooLow()',
        executeRequest([route, amountIn, minimumGrossProfit - 1n, block.timestamp + 300n]),
      ),
    )
    negativeChecks.push(
      await mustRevert(
        ...
        'Expired',
        'Expired()',
        executeRequest([route, amountIn, protectedProfit, block.timestamp - 1n]),
      ),
    )
    negativeChecks.push(
    ...
    negativeChecks.push(
      await mustRevert(
        ...
        'InvalidV3Path()',
        executeRequest([
          { ...route, entryV3Path: v3Path([USDG, ENTRY_TOKEN], [500]) },
          amountIn,
          protectedProfit,
    ...
    negativeChecks.push(
      await mustRevert(
        ...
        'MissingV3Pool()',
        executeRequest([
          { ...route, entryV3Path: v3Path([WETH, USDG, ENTRY_TOKEN], [3_000, 500]) },
          amountIn,
          protectedProfit,
          block.timestamp + 300n,
        ]),
    ...
    negativeChecks.push(
      await mustRevert(
        ...
        'InvalidV3Path()',
        executeRequest([
          { ...route, entryV3Path: v3Path([WETH, USDG, TARGET_TOKEN, ENTRY_TOKEN], [100, 500, 500]) },
          amountIn,
          protectedProfit,
          block.timestamp + 300n,
        ]),
    ...
    negativeChecks.push(
      await mustRevert(
        'two_hop_non_usdg_bridge',
        'InvalidV3Path',
        'InvalidV3Path()',
        executeRequest([
          { ...route, entryV3Path: v3Path([WETH, TARGET_TOKEN, ENTRY_TOKEN], [100, 500]) },
          amountIn,
          protectedProfit,
          block.timestamp + 300n,
        ]),
        ...
        executeRequest([
          {
            ...
            exitV3Path: v3Path([ENTRY_TOKEN, USDG, WETH], [500, 100]),
    ...
    negativeChecks.push(
      await mustRevert(
        ...
        executeRequest([route, amountIn, protectedProfit, block.timestamp + 300n]),
      ),
    )
    await write(V3_ROUTER, routerAbi, 'configure', [expectedProfit])
    ...
    if (
      executeReceipt.status !== 'success' ||
      directReceipt.status !== 'success' ||
      balances.weth !== seed + expectedProfit * 2n
    ) {
      throw new Error('confirmed WETH executions did not increase WETH by the exact simulated profit')
    }
    if (
      balances.usdg !== 0n ||
      balances.entryToken !== 0n ||
      balances.targetToken !== 0n ||
      balances.exitToken !== 0n
    ) {
      throw new Error('confirmed WETH execution left an intermediate asset balance')
    }
    if (allowances.weth !== 0n || allowances.exitToken !== 0n) {
      throw new Error('confirmed WETH execution left a router allowance')
    }

    console.log(
      JSON.stringify({
        status: 'WETH_DETERMINISTIC_CONTRACT_TEST_PASSED',
        executor,
        sourceHash: keccak256(toHex(fs.readFileSync(path.join(root, 'contracts', 'WethAtomicArb.sol'), 'utf8'))),
        bounds: {
          maximumAmountIn: maximumAmountIn.toString(),
          minimumGrossProfit: minimumGrossProfit.toString(),
        },
        seed: {
          nativeIn: seed.toString(),
          wethPrincipal: seededWeth.toString(),
          strandedNative: strandedNative.toString(),
        },
        ...
        confirmed: {
          ...
          executorWeth: balances.weth.toString(),
        },
        residuals: {
          usdg: balances.usdg.toString(),
          entryToken: balances.entryToken.toString(),
          targetToken: balances.targetToken.toString(),
          exitToken: balances.exitToken.toString(),
        },
        allowances: { weth: allowances.weth.toString(), exitToken: allowances.exitToken.toString() },
        negativeChecks,
      }),
    )
  } catch (error) {
