...
9. 本次 v0.16.10 将安全审计读取改为增量缓存，并增加 Earn 托管 WSS 与公共优先的受限 RPC 回退。
...
1. 增量读取安全审计事件，保留完整追加账本；
2. 截断并脱敏外部错误，阻断 HTML 错误正文进入每轮堆分配；
3. Earn 通过托管 WSS 接收 Vault Swap，公共 HTTP 仅 60 秒恢复；
4. 公共 RPC 403 后允许托管 HTTP 回退，但用每次和每日双重硬预算；
5. 授权升级到 v12，部署时必须 disarm、对账、runtime verify、重新 arm；
6. 生产浸泡同时核对 RSS/heap、服务状态、WSS、RPC 用量、nonce、余额和未决交易；
