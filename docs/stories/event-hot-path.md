...
- One event cycle quotes at most one candidate, two amounts per enabled base, one V4 pair and one previously proven V3
  topology per amount.
...
- Event reads use two logical attempts with a 200 ms retry delay; periodic reconciliation keeps its broader retry
  policy.
...
- Unit tests cover event revision preemption, stale-backlog eviction, newest-first ordering, touched-route selection,
  bounded V3 topology, event-specific retry selection and bounded amount selection.
- Production evidence records event latency, logical Quoter calls, public-RPC health, service restarts and signer usage.

## Out of scope

- A claim that the one-pair/one-V3-topology/two-amount hot probe is route-complete.
