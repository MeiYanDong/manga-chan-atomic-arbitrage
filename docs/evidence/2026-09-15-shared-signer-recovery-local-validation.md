...
- `manga-critical-health.service` 在 64 MiB 内存上限下反复 ABRT。旧入口为复用两个小型校验函数加载了
  `business-operations.mjs`、`journal.mjs` 及其链上依赖，因此停机提醒未能可靠送达；
...
4. 飞书检查器依赖过重且文案暴露内部术语，没有优先说明影响、系统动作和用户是否需要处理。
...
- 关键提醒器改为无第三方依赖的小模块，systemd 内存上限调整为 128 MiB。
