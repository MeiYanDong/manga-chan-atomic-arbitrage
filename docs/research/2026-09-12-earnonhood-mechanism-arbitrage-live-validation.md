...
- 区块 `60494886`：执行后的全池扫描虽找到多条毛收益为正的路径，但 12 条最高毛收益 WETH 样本中仅两条在
  缓冲 Gas 后仍为正；其中较好的一条 `WETH -> MOO -> WETH` 也只剩 `0.000005343462727825 ETH`，低于本策略
  的最低净收益和余量门，不应再次实盘。

这说明核心机会是**短时、薄深度、会换方向的状态不一致**。手工发现后再操作太慢；一次成功也不足以推断每小时
频率、胜率、容量上限或月收益。

## 6. 执行保护（2026-09-14 更新）

历史首笔验证只支持写死的 AI/MOO 路径；该限制不是套利机制的一部分。v0.13.1 实盘执行器改为直接读取官方
Omnipool Factory、明确复核的历史池与各池链上状态，枚举全部 2～4 跳 WETH 闭环，再以本地近似粗排和有界
exact quote 选出候选。网页 API 不再是签名关键路径，代币名称也不进入授权白名单；每次预检仍会：

- 核对 chain ID、Vault、BatchRouter、WETH，以及入选池的 bytecode、完整 token 集、初始化/暂停/恢复状态和
  `0.30%` 费率；
...
动态全集并不意味着对所有路径无限请求 RPC：全图先在本地计算，最多 24 条路径进入粗报价、最多 8 条进入
细化，公共 exact quote 上限为 120；公共阶段净收益为正后，受控执行 RPC 只复核同一条路径、最多 9 个金额。
最终路径、池、方向和金额被写入不可变 mutation plan。`EARN_LIVE_ARM=1` 只对一次命令生效，不保存在配置中。
...
当前生产形态不是另起一个并行签名机器人，而是把 Earn Omnipool 动态图接入现有唯一签名器：

```text
公共 RPC / Earn Factory 链上发现
  -> 链上身份、余额、权重和事件核验
  -> 加权公式低成本粗排
