...
const CONFIG_KEYS = new Set([
  ...
  'GLOBAL_WATCH_CHILD_TIMEOUT_MS',
  'EARN_WATCH_CHILD_TIMEOUT_MS',
  'EARN_MANAGED_FALLBACK_DAILY_LOGICAL_CALL_CAP',
...
export function loadRuntimeConfig(environment = process.env) {
  ...
  return {
    ...
    earnLiveCoarseProbePoints: boundedInteger(value('EARN_LIVE_COARSE_PROBE_POINTS'), 8, 4, 16),
    earnLiveRefinementPoints: boundedInteger(value('EARN_LIVE_REFINEMENT_POINTS'), 6, 2, 16),
    earnWatchEnabled: strictBoolean(value('EARN_WATCH_ENABLED'), false),
    earnWatchEventPollMs: boundedInteger(value('EARN_WATCH_EVENT_POLL_MS'), 60_000, 1_000, 60_000),
    ...
    globalWatchChildTimeoutMs: boundedInteger(value('GLOBAL_WATCH_CHILD_TIMEOUT_MS'), 60_000, 15_000, 180_000),
    earnWatchChildTimeoutMs: boundedInteger(value('EARN_WATCH_CHILD_TIMEOUT_MS'), 60_000, 15_000, 180_000),
    earnManagedFallbackDailyLogicalCallCap: boundedInteger(
      value('EARN_MANAGED_FALLBACK_DAILY_LOGICAL_CALL_CAP'),
      40_000,
      1_000,
      1_000_000,
    ),
