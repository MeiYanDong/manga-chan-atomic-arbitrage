...
import {
    IERC20GenericMinimal,
    IPoolManagerGenericMinimal,
    IUniswapV3FactoryGenericMinimal,
    IUniswapV3PoolGenericMinimal,
    IUniswapV3RouterGenericMinimal
} from "./GenericAtomicArb.sol";

interface IWethDepositMinimal {
    function deposit() external payable;
}

/// @title Bounded WETH-based PAIR atomic-arbitrage executor
/// @notice Executes one operator-selected cycle:
/// WETH -(V3, zero to two hops)-> entry quote -(PAIR V4)-> target
///      -(PAIR V4)-> exit quote -(V3, zero to two hops)-> WETH.
/// Native ETH is accepted only as deployment seed and is wrapped immediately;
/// runtime principal and profit accounting remain ERC-20 WETH balance deltas.
contract WethAtomicArb {
    ...
    uint256 public immutable MIN_GROSS_PROFIT;
    ...
    uint256 public constant HARD_MAX_AMOUNT_IN = 1 ether;
    ...
    bytes32 private constant PHASE_SLOT = keccak256("weth.arb.phase.v1");
    bytes32 private constant EXPECTED_POOL_SLOT = keccak256("weth.arb.pool.v1");
    bytes32 private constant EXPECTED_TOKEN_SLOT = keccak256("weth.arb.token.v1");
    bytes32 private constant EXPECTED_AMOUNT_SLOT = keccak256("weth.arb.amount.v1");
    bytes32 private constant INPUT_TOKEN0_SLOT = keccak256("weth.arb.token0.v1");

    constructor(address operator_, uint128 maxAmountIn_, uint128 minimumGrossProfit_) payable {
        if (operator_ == address(0)) revert NotOperator();
        if (
            maxAmountIn_ == 0 || maxAmountIn_ > HARD_MAX_AMOUNT_IN || minimumGrossProfit_ == 0
                || minimumGrossProfit_ > maxAmountIn_ || msg.value > maxAmountIn_
        ) revert InvalidBounds();
        operator = operator_;
        MAX_AMOUNT_IN = maxAmountIn_;
        MIN_GROSS_PROFIT = minimumGrossProfit_;

        if (msg.value != 0) {
            uint256 balanceBefore = IERC20GenericMinimal(WETH).balanceOf(address(this));
            IWethDepositMinimal(WETH).deposit{value: msg.value}();
            uint256 balanceAfter = IERC20GenericMinimal(WETH).balanceOf(address(this));
            if (balanceAfter != balanceBefore + msg.value) revert SettlementMismatch();
        }
    }
    ...
    /// @notice Executes a typed, bounded WETH route. The sender should set
    /// `minProfit` high enough to cover worst-case native Gas plus desired net.
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        if (route.entryToken == WETH) {
            _safeTransfer(WETH, POOL_MANAGER, amountIn);
            ...
            _setDirectContext(PHASE_ENTRY_DIRECT, entryPool, WETH, amountIn, zeroForOne);
            ...
            _forceApprove(WETH, V3_ROUTER, amountIn);
        ...
        uint256 output;
        if (route.exitToken == WETH) {
            manager.take(WETH, address(this), exitAmount);
            output = exitAmount;
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata) external {
        ...
        if (phase == PHASE_ENTRY_DIRECT) {
            _safeTransfer(WETH, msg.sender, uint256(inputDelta));
        } else {
    ...
    function _validateRoute(Route calldata route) private view {
        if (
            route.targetToken == address(0) || route.entryToken == address(0) || route.exitToken == address(0)
                || route.targetToken == WETH || route.targetToken == USDG || route.targetToken == route.entryToken
        ...
        _validateV3Path(route.entryV3Path, WETH, route.entryToken);
        _validateV3Path(route.exitV3Path, route.exitToken, WETH);
    }
    ...
    function _validateV3Path(bytes calldata path, address expectedStart, address expectedEnd) private view {
        ...
        for (uint256 index = 0; index < hops; ++index) {
            ...
            if (hops == 2 && index == 0 && nextToken != USDG) revert InvalidV3Path();
    ...
    function _expectedPool() private view returns (address value) {
    ...
    function _expectedToken() private view returns (address value) {
    ...
    function _expectedAmount() private view returns (uint256 value) {
    ...
    function _inputToken0() private view returns (bool value) {
